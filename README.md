...
**Ядро sing-box ставить НЕ нужно** — оно встроено в приложение (Tauri sidecar,
`bundle.externalBin`). Скачал, открыл — работает. Бинарь добавляется при сборке
скриптом `scripts/fetch-sing-box.sh` (вызывается из `beforeBuildCommand`).

> Опытные пользователи могут поставить свой `sing-box` (`brew install sing-box`)
> — клиент возьмёт встроенный, а при его отсутствии (например, в `tauri dev`)
> ищет в `/opt/homebrew/bin`, `/usr/local/bin`, `$PATH`.

> **Платформы.** Десктоп (macOS/Windows/Linux) — один код на Tauri. Для Android есть отдельное нативное приложение — [**coffeeNetwork-android**](https://github.com/Arti-Ko/coffeeNetwork-android): тот же дизайн и логика роутинга, но на Flutter, потому что на Android sing-box работает только через `VpnService` + libbox.
...
- **coffeeNetwork** (этот репозиторий) — десктопный клиент для macOS / Windows / Linux на Tauri.
- [**coffeeNetwork-android**](https://github.com/Arti-Ko/coffeeNetwork-android) — нативный Android-клиент на Flutter + sing-box (`VpnService` + libbox). Тот же дизайн «посадочного талона» и та же логика обхода РФ.
- [**NetForge**](https://github.com/Arti-Ko/netforge) — серверная половина: фабрика конфигов Hysteria2. Заводит пользователей на VPS по SSH и выдаёт готовые ссылки, которые импортируешь в клиенты выше.
